...
export async function userRigister(
...
export async function getUserProfile(
  ...
  return request<API.BaseResponseUserVO>(`/user/${params.id}/profile`, {
    method: 'GET',
    ...(options || {}),
  })
}
...
/** 此处后端没有提供注释 POST /user/update/status */
export async function updateUserStatus(
  body: API.UpdateUserStatusRequest,
  options?: { [key: string]: any }
) {
  return request<API.BaseResponseBoolean>('/user/update/status', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
